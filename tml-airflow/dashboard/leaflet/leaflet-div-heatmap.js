...
L.DivHeatmapLayer = L.FeatureGroup.extend({
  ...
  _addBlob: function(lat,lng,value,old_marker,pmess){
    // Remove previous
  ...
  //open popup;
	var popup = L.popup({ minWidth: 600, maxWidth: 1000, maxHeight: 180 })
   .setLatLng([lat, lng]) 
   .setContent(pmess)
   .openOn(map);
	});
  ...
  setData: function(data,pmess) {
    // Data object is three values [ {lat,lon,value}, {...}, ...]
    //this.clearData();
    var self = this;
    data.forEach(function(point){
      point.value = point.value > 1 ? 1 : point.value;
	//  console.log("point value=",point.lat,point.lon,point.value);
      var marker = self._addBlob(point.lat,point.lon,point.value,marker,pmess);
  ...
  */
  testRandomData: function(number) {
    ...
    while (count-- > 0) {
      ...
      data.push({
        "lat": 68.70950672693772,
        "lon": 112.66600273882062,
        "value": 0.26
      });
    }
    return data;
  },
  testRandomData2: function(coord,c) {
    var data = [];
    var count = c;
    while (count-- > 0) {
      var size = Math.round(Math.random() * 150);	// in pixels
  ...
  testAddData: function(coord,value,pmess) {
    //this.clearData();
	//console.log("data=",this.testRandomData(number));
	
	var data=this.testRandomData2(coord,value)
	//console.log(data);
	
    this.setData(data,pmess);
	//this.setData([{"lat":68.70950672693772, "lon":112.66600273882062,"value":0.16647315308833965},{"lat":68.70950672693772, "lon":112.66600273882062,"value":0.16647315308833965}]);
